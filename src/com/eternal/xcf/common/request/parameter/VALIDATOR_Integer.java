 ...
 *
 */
public class VALIDATOR_Integer implements XCFValidator {

	public boolean validate(XCFRequest req,	XCFProcessingInstruction parameterSpecification)
			throws XCFException {
		boolean isInt = req.parameterExists(parameterSpecification.getName());
		if (isInt) try {
			Integer.parseInt(req.getParameter(parameterSpecification.getName()));
			isInt = true;
